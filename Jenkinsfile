pipeline {
    ...
    stages{
        ...
        stage('Build'){
            steps{
                sh 'sudo docker build . -t blueasni/nodo-todo-app-test:latest'
            }
        }
        stage('Test image') {
            steps {
                echo 'testing...'
                sh 'sudo docker inspect --type=image blueasni/nodo-todo-app-test:latest '
            }
        }
        
        stage('Push'){
            steps{
        	     sh "sudo docker login -u blueasni -p dckr_pat_OvN0lH_USJztUCkm0opyjz-yXNc"
                 sh 'sudo docker push blueasni/nodo-todo-app-test:latest'
            }
        }  
        stage('Deploy'){
            steps{
